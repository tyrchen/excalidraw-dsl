...
fn test_invalid_edsl_syntax() {
    ...
    assert!(matches!(error, excalidraw_dsl::EDSLError::Parse(_)));
}
...
";
    ...
    assert!(result.is_err());
    let error = result.err().unwrap();
    assert!(matches!(error, excalidraw_dsl::EDSLError::Build(_)));
}
...
";
    ...
    assert!(result.is_err());
    let error = result.err().unwrap();
    assert!(matches!(error, excalidraw_dsl::EDSLError::Layout(_)));
